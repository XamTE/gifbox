...
자주 가는 사이트를 등록해두면 창 위쪽 줄에서 한 번에 엽니다. 설정 창에서
추가·수정·삭제합니다.

**창 안에 브라우저를 심지 않고 기본 브라우저를 부릅니다.** 로그인 세션과 확장
프로그램이 그대로 살아 있고 exe 도 커지지 않기 때문입니다. GifBox 를 '항상 위'로
띄워두면 브라우저에서 이미지를 끌어다 바로 떨어뜨릴 수 있어서, 하려던 일에는
오히려 이쪽이 잘 맞습니다.

### 패치노트 · 업데이트

새 버전으로 처음 실행하면 바뀐 점을 한 번 보여줍니다('다시 보지 않기'로 끌 수
있음). 패치노트는 exe 에 동봉한 `CHANGELOG.md` 에서 읽습니다 — 네트워크가 없어도
보이고, 무엇보다 **지금 실행 중인 빌드의 내용**과 항상 일치하기 때문입니다.

설정 창에서 새 버전을 확인할 수 있습니다. 하루 한 번만 조용히 확인하고 실패해도
방해하지 않습니다. **받은 파일로 자기 자신을 덮어쓰지는 않습니다** — 실행 중인 exe
는 윈도우가 잠가두고, 서명 없는 exe 의 자기 교체는 백신이 정확히 잡는 패턴이라
내려받아 탐색기로 열어주는 데까지만 하고 마지막 한 걸음은 사람이 합니다.
...
gifbox/
  ...
  bookmarks.py   즐겨찾기 (기본 브라우저로 열기 · 주소 검증)
  updater.py     GitHub 릴리스 확인 · 내려받기 · 체크섬 검증
  changelog.py   CHANGELOG.md 에서 해당 버전 항목 뽑기
  notes_dialog.py     패치노트 창
...
- **버전은 `gifbox/__init__.py` 의 `__version__` 한 곳에서만 정합니다.** 태그를 밀면
  CI 가 태그와 이 값이 같은지, `CHANGELOG.md` 에 그 버전 항목이 있는지 검사하고
  어긋나면 빌드를 깹니다. (한때 코드가 1.0.0 인 채로 v1.1.0 이 나간 적이 있습니다)
